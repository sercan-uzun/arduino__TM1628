...
const byte seg_addr[5]={0x02,0x03,0x07,0x04,0x05};//no bit of digital segments
//                         SG0  SG1  SG2  SG3  SG4  SG5  DC5  SG7  DC2  DC1  MP3  DC3  DC6  DC4  VCD  DTS  DDD  CL1  CL2   //name   -|
...
//                       DVD  PLY  PAU  CD_  DC1  DC2  DC3  DC4  DC5  DC6  CL1  CL2  VCD  DTS  DDD   //name   -|
const byte led_addr[15]={0x01,0x03,0x09,0x07,0x03,0x01,0x09,0x07,0x0D,0x0b,0x0B,0x02,0x00,0x08,0x06};//adress -| for the signs's and disc's leds
const byte led_val[15]= {0x01,0x01,0x01,0x01,0x02,0x02,0x02,0x02,0x02,0x02,0x01,0x40,0x40,0x40,0x40};//byte   -|
...
void TM1628::setDisc(boolean onoff){
	if (onoff) {
		for(int i=4;i<10;i++)// turn on all disc segments
